...
export default function Drive({
  ...
  const onUpload = useCallback(
    async (ev: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (let i = 0; i < picked.length; i++) {
        ...
        try {
          const { id, uploadUrl } = await call("/api/files", {
            method: "POST",
            body: JSON.stringify({
              folderId,
              name: file.name,
          ...
          await call(`/api/files/${id}/confirm`, { method: "POST" });
        } catch (e) {
          setError(
  ...
  const folderMenu = useCallback(
    ...
    [isAdmin, enter, addFolder, triggerUpload, renameNode, deleteNode, openMenu]
  );
  ...
  const fileMenu = useCallback(
    ...
    [isAdmin, downloadFile, renameFileAction, deleteFileAction, openMenu]
  );
  ...
  const folders = entries.map((e, idx) => {
