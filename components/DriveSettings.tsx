 ...
 *
 *   - not signed in: ask for the owner passcode. An admin is additionally
 *     offered the drive's seat without one, which is a shortcut for something
 *     they can already do rather than a way around anything.
...
export default function DriveSettings({
  ...
  isOwner: boolean;
  /** Only decides whether the take-over shortcut is offered. */
  isAdmin: boolean;
  onClose: () => void;
}) {
  return (
    ...
    <div style={{ padding: "20px 18px 22px" }}>
      ...
      {isAdmin && (
        <>
          ...
          <div style={{ ...LABEL, marginBottom: 8 }}>As the admin</div>
          <p style={{ margin: "0 0 12px", fontSize: 13, opacity: 0.75 }}>
            You can take this drive&rsquo;s seat without its passcode — you set that passcode, so
            there is nothing here to get around. It is asked for rather than assumed, so
            administering the drives does not silently mean running all of them.
          </p>
          <button
            className="btn btn-secondary btn-block"
            onClick={() => void send({ takeOver: true })}
            disabled={busy}
            style={{ justifyContent: "center", marginTop: 0 }}
          >
            <Icon name="shield" size={14} />
            {busy ? "Taking the seat…" : "Take the owner's seat"}
          </button>
        </>
      )}
    </div>
