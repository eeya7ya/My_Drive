...
-- One minute of meeting per row, written by the open /MOM route. `document`
-- is the whole minute as JSON; the columns beside it are copies of the fields
-- the picker lists, so listing never reads a document (or an embedded logo)
-- it is not going to show. See migrations/007_minutes.sql.
