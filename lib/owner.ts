/**
 * "Whose drive is this row in, and are you in that drive?"
 *
 ...
 *
 * What it then asks is `requireDriveUser`: being in a drive is what makes it
 * yours to run, and the admin decides who is in it. See lib/auth.ts.
 *
 * A row that does not exist is a 404 and is answered before the permission
 * question, because "no such file" is not something a passcode would change
 * and the alternative — 403 for anything outside your drives, including things
 * that are not there — would turn these routes into a way of asking which ids
 * exist.
 */
 ...
 * since its refusal names the drive. A row whose drive key matches no registry
 * row is treated as missing: it cannot be managed by anyone, and saying so is
 * better than a 500.
 */
...
/** Throws unless the caller is in the drive this folder is in. */
...
/** Throws unless the caller is in the drive this file is in. */
...
/** Throws unless the caller is in the drive this revision's file is in. */
