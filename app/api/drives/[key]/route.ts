...
function assertFieldsAllowed(
  ...
  badRequest(
    role === "owner"
      ...
      : `${forbidden.join(", ")} ${forbidden.length === 1 ? "belongs" : "belong"} to the drive's owner. Take the owner's seat for this drive to change ${forbidden.length === 1 ? "it" : "them"}.`
  );
}
 ...
 *
 * The owner is asked about first, because an admin who has taken a drive's
 * owner seat should be treated as its owner here rather than being told their
 * own drive's name is somebody else's to set.
 */
