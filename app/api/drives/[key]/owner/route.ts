...
/**
 * Sign in as one drive's owner.
 *
 * One way in: the drive's owner passcode. The admin has no shortcut past it —
 * they set that passcode, and if they need to work inside the drive they use
 * it like anybody else. Wrong guesses are counted against the throttle.
 */
...
export async function GET(_req: Request, { params }: Ctx) {
  try {
    ...
    const brand = await getDrive(key);
    if (!brand) refused();
    return ok({ hasOwner: brand.hasOwner, isAdmin: await isAdmin() });
  } catch (err) {
    return fail(err);
  }
}
