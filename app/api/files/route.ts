 ...
 * drive: without a pass the upload is refused before a URL is ever signed.
 */
export async function POST(req: Request) {
  try {
    const { drive: rawDrive, folderId, name, size, contentType } = await readJson<{
      drive?: string;
    ...
    const { usedBytes, quotaBytes } = await getUsage(brand.key);
